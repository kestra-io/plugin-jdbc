...
# 2) Start containers
###############################################################################
$COMPOSE up --quiet-pull -d mariadb sqlserver postgres clickhouse druid_postgres druid_zookeeper druid_coordinator druid_broker druid_historical druid_middlemanager druid_router
...
#
## db2 (5023)
#echo "Waiting for DB2 setup to complete (log marker)..."
#wait_for "db2-setup-complete" \
#  "docker compose -f docker-compose-ci.yml logs db2 | grep -q 'Setup has completed'" \
#  1200 5
#
## Ensure the host port is listening before running tests.
#wait_for_tcp "db2-tcp" 127.0.0.1 5023 600
