...
@NoArgsConstructor
@Schema(
    title = "Execute multiple SQL statements against Actian Vector",
...
@Plugin(
    aliases = "io.kestra.plugin.jdbc.vectorwise.Queries",
    examples = {
        @Example(
            title = "Send SQL queries to an Actian Vector database and fetch a row as output.",
            full = true,
            code = """
...
public class Queries extends AbstractJdbcQueries implements ActianVectorConnectionInterface {
    @Override
    protected AbstractCellConverter getCellConverter(ZoneId zoneId) {
        return new ActianVectorCellConverter(zoneId);
    }
