...
@NoArgsConstructor
@Schema(
    title = "Wait for query results on Dremio and trigger flow",
    description = "Periodically polls Dremio by executing a SQL query at the specified interval (default 60 seconds). Triggers a downstream flow execution when the query returns one or more rows. Supports parameterized queries and afterSQL for marking processed rows. Use fetchType to control result handling."
)
@Plugin(
    examples = {
        @Example(
...
public class Trigger extends AbstractJdbcTrigger implements DremioConnectionInterface {
    @Override
